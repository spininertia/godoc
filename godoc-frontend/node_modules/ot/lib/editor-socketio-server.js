...
function EditorSocketIOServer (document, startSeqNum, operations, docId, mayWrite) {
...
EditorSocketIOServer.prototype.addClient = function (socket, option) {
  var self = this;
  socket
    .join(this.docId)
    .emit('doc', {
    ...
    .on('operation', function (revision, operation, cursor) {
      self.mayWrite(socket, function (mayWrite) {
        ...
        self.onOperation(socket, revision, operation, cursor, option);
      });
    })
...
EditorSocketIOServer.prototype.onOperation = function (socket, revision, operation, cursor, option) {
  ...
  try {
    ...
    option['seq'] = this.startSeqNum;
    this.startSeqNum += 1;
    this.receiveOperation(revision, wrapped, this, option, function (wrappedPrime, o) {
      o.getClient(clientId).cursor = wrappedPrime.meta;
      socket.emit('ack');
      socket.broadcast['in'](o.docId).emit(
        'operation', clientId,
